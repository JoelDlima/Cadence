...
The schema file is `Cadence/supabase/schema.sql`. It creates four tables with
...
#   SQL Editor -> New query -> paste Cadence/supabase/schema.sql -> Run
```
...
If you see `"sync_state": "error"`, check `Cadence/logs/api.err` for the HTTP
...
A 2026 research summary is in `Cadence/docs/cloud-mirror.md`. Short version:
