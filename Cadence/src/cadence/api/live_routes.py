...
class LiveCustomerIn(BaseModel):
    ...
    email: str = "customer@cadence.local"
...
def create_live_router(*, app: FastAPI, db, runtime) -> APIRouter:
        ...
        if body.attach_pdf:
            try:
                ...
                if journey is None:
                    ...
                    story.append(Paragraph(
                        "Generated by Cadence (Autonomous Revenue Recovery)",
                        styles["Italic"],
                    ))
                    doc.build(story)
    ...
    def _emit_synthesized_hinglish(journey: dict, outcome_label: str, channel: str = "email") -> None:
        """Write a fallback Hinglish body into the audit chain so the SPA's
        audio card, email card, and chat-reasoning panel all have something
        to render even on journeys that didn't go through the full agent
        pipeline. Called by every lifecycle path on success."""
        es = runtime.store
        body = (
            f"Namaste! Your {journey.get('amount_minor', 49900) / 100:.0f} rupees "
            f"subscription for {journey.get('subscription_id', 'your account')} "
            f"is now marked {outcome_label}. Reply STOP to opt out. - Cadence"
        )
        es.append(
            event_type="agent.thinking",
            aggregate_type="journey", aggregate_id=journey.get("id", "unknown"),
            payload={
                "agent": "lifecycle_helper",
                "channel": channel,
                "kind": "synthesized",
                "outcome": outcome_label,
                "body": body,
                "subject": "Action needed on your Cadence subscription",
                "reason": "engine path did not run message_writer; "
                          "this fallback ensures the SPA has a body to render",
            },
            occurred_at=utc_iso(datetime.now(timezone.utc)),
            recorded_at=utc_iso(datetime.now(timezone.utc)),
            event_id=f"evt_lc_synth_{uuid.uuid4().hex[:10]}",
        )
