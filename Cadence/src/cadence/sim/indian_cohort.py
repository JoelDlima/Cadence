...
``cadence.sim.cohort.generate_cohort``); this file provides an *opt-in*
...
- ``cohort`` is a ``list[SimSubscriber]`` identical in shape to what
  ``cadence.sim.cohort.generate_cohort`` returns, so the existing
  ``run_arm_naive`` and ``run_arm_cadence`` consumers take this list as a
  drop-in replacement.
...
from cadence.classify.taxonomy import UNKNOWN
from cadence.sim.cohort import (
    SimSubscriber,
    _FAILURE_CODES,
...
# recovery-rate table in cadence.sim.outcomes is calibrated against this mix.
_AMOUNT_TIERS = (
    (19900, 30),
    (29900, 25),
    (49900, 20),
