"""Phase D evaluation harness: naive baseline arm vs the real Revive machinery.

Both arms run on the SAME seeded cohort with the SAME calibrated outcome table
(``revive.sim.outcomes``), so the comparison is apples-to-apples:
...
- Revive arm: real engine, Guardian, durable queue, timers, executors on a
...
from revive.config import ChannelConfig, PolicyConfig
...
from revive.sim.cohort import SimSubscriber, generate_cohort, root_cause_of, webhook_payload
from revive.sim.outcomes import outcome_for
from revive.store.db import Database
from revive.store.event_store import EventStore
from revive.store.journey_repo import STATE_RECOVERED, Journey, JourneyRepo
...
def run_arm_naive(cohort: list[SimSubscriber], workdir: Path | str) -> ArmResult:
    """Blind baseline WITHOUT Revive machinery: retry +24h then d1/d3/d5 emails."""
...
def _offline_email_cfg() -> ChannelConfig:
    return ChannelConfig(resend_api_key="", email_from="revive@example.com")


def run_arm_revive(cohort: list[SimSubscriber], workdir: Path | str) -> ArmResult:
    """Run the REAL Revive machinery end to end over the cohort on a fresh DB."""
    workdir = Path(workdir)
    workdir.mkdir(parents=True, exist_ok=True)
    db_path = workdir / "revive.db"
    for stale in (db_path, Path(f"{db_path}-wal"), Path(f"{db_path}-shm")):
        if stale.exists():
            stale.unlink()
    db = Database(db_path)
    try:
        return _run_revive_on(cohort, db)
    finally:
        db.close()


def _run_revive_on(cohort: list[SimSubscriber], db: Database) -> ArmResult:
    clock = FakeClock()
...
def _report_text(metrics: dict[str, Any], *, n: int, seed: int, out_dir: Path) -> str:
    naive, revive, uplift = metrics["naive"], metrics["revive"], metrics["uplift_pct"]
    ...
    rows = "\n".join(
        ...
        for name, m in (("naive", naive), ("revive", revive))
    )
    return f"""# Revive Evaluation Report

Date: {today} · Cohort: {n} synthetic subscribers · Seed: {seed} · Arms: naive vs revive
...
Relative recovery-rate uplift (revive vs naive): **{uplift:+.1f}%**.

**Zero policy violations**: every executed action passed the Guardian pre-action veto layer
({revive['vetoes']} vetoes fired at caps/windows/hard-decline stops; 0 illegal actions executed).
...
def run_experiment(n: int = 500, seed: int = 42, out_dir: Path | str = "docs") -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="revive_eval_") as tmp:
    ...
    revive_metrics = _arm_metrics(revive, len(cohort))
    base_rate = float(naive_metrics["recovery_rate_pct"])
    lift = float(revive_metrics["recovery_rate_pct"]) - base_rate
