...
Resend, and LLM keys into `Cadence/.env`. It is the inverse of `docs/PITCH-VIDEO.md`'s
...
keys in `Cadence/.env` activates the live code path for that dependency;
...
cd ../Cadence/frontend && npm install && npm run dev &
...
(openssl rand -hex 32). The Edge Function in `Cadence/supabase/functions/revive-ingest/index.ts`
...
**What to put in `Cadence/.env` (Groq):**
...
2. In SQL Editor → New query → paste the entire contents of
   `Cadence/supabase/schema.sql` → Run. This creates 4 tables with
...
**If it says "error":** check `Cadence/logs/api.err` for the HTTP status
...
# In three terminals
python -m uvicorn revive.api.app:app --port 8000
cd ../Cadence/frontend && npm run dev
...
cd Cadence && python -m uvicorn revive.api.app:app --port 8000 &
...
|---|---|---|
| `/api/status` shows `mode: "DEMO"` after setting keys | `.env` not in the right place | File must be `Cadence/.env`, not `Cadence/Cadence/.env` or `.env`. |
...
| Supabase status shows `error` after deploy | Schema not applied | Run `Cadence/supabase/schema.sql` in SQL Editor |
...
`Cadence/logs/api.log` (uvicorn stdout) and `Cadence/logs/api.err` (stderr).

---

## 8. To go back to DEMO

Comment out the keys in `Cadence/.env` (or rename the file to `.env.live`
