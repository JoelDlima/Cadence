...
One page, judge-readable. Every box below exists in `Cadence/src/revive/`
...
`Cadence/.env` activates the live code path for that dependency only; the
...
| Demo must work on a fresh clone | Every external dependency has a keyless simulator. `Cadence/.env` keys activate the live path for that one dependency; the keyless path is unchanged. |
