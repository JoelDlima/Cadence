...
To switch from DEMO to LIVE, fill `Cadence/.env` with the real Razorpay
...
`Cadence/.env` per `docs/KEYS-DAY.md` and the SPA flips to LIVE.
...
`Cadence/supabase/schema.sql` and creates four tables (`webhook_inbox`,
...
wrote `Cadence/src/revive/sim/indian_cohort.py` (`generate_indian_cohort(n,
seed)` with `hi_IN` locale, realistic Indian names, UPI handles, IFSC
codes from a known Indian bank set), `Cadence/scripts/run_eval_indian.py`,
updated `/api/eval-summary` to prefer `docs/eval-metrics-large.json` when
present, and added `Cadence/tests/test_indian_cohort.py` (6 tests covering
...
`Cadence/src/revive/observability/phoenix.py` is a graceful no-op when
...
`Cadence/docs/phoenix-setup.md` is the 30-second setup walkthrough. Tests
...
`Cadence/src/revive/agents/llm_client.py` (Sarvam is OpenAI-compatible at
...
keep working. Updated `Cadence/.env.example` with the Sarvam block. Two
...
`Cadence/src/revive/policy/circulars.py` with heuristic extractors
(source detection, summary, date, reference, rule list capped at 32).
New `Cadence/src/revive/store/V3__policy_circulars.sql` migration adds
...
New `Cadence/tests/test_adversarial_guardian.py` (50 tests, 360 total in
...
tokens**. Updated `Cadence/src/revive/classify/taxonomy.py` to add
...
artifact: removed `Cadence/-` (a stray 36-byte file created by a bad shell
command earlier).

**Phase A (shipped) — Recovery Brain (deterministic AI bandit).** A deterministic
contextual bandit in `Cadence/src/revive/policy/bandit.py` that scores
...
`Cadence/src/revive/policy/nudge_templates.py` module with
...
tracker is `Cadence/src/revive/agents/ptp_parser.py`: a deterministic,
...
direction #2. A new `Cadence/src/revive/store/V4__checkout.sql`
table (`checkout_sessions`) and a pure-function state machine
in `Cadence/src/revive/checkout/recovery.py` (ladders: OPEN ->
...
`Cadence/src/revive/store/V5__b2b.sql` (`b2b_invoices` +
`b2b_orgs`) and `Cadence/src/revive/b2b/chaser.py` state
...
`Cadence/src/revive/mandate/sequencer.py` (ladder: 3+ distinct
...
example direction #6. A new `Cadence/src/revive/policy/voice_tts.py`
