...
# Usage:  ./Cadence/scripts/dev.sh   (or:  bash Cadence/scripts/dev.sh)
#
# Brings up:  FastAPI on :8000, Vite dev server on :3000
# Requires:   Python 3.12+, Node 20+, pip, npm
# Side effects: creates Cadence/.venv, Cadence/data/, frontend/node_modules on first run.
...
# Start frontend
cd "$REPO_ROOT/Cadence/frontend"
if [ ! -d "node_modules" ]; then
  echo "==> installing frontend deps (this can take a minute)"
  npm install
fi
echo "==> starting Vite on http://127.0.0.1:3000"
nohup npm run dev -- --host 127.0.0.1 --port 3000 > "$REPO_ROOT/Cadence/logs/web.log" 2>&1 &
WEB_PID=$!
...
================================================================
 Cadence is up.
   ...
   API logs : Cadence/logs/api.log
   Web logs : Cadence/logs/web.log

   Cadence is running in DEMO mode (no Razorpay keys needed).
   Add keys to Cadence/.env to switch to LIVE mode.
