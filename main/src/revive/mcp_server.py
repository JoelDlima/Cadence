...
Tools (8 total):
    ...
    - revive_get_metrics: control-room totals (recovered INR, state counts, etc.)
    - revive_list_dead_letters: tasks that exhausted retries
    - revive_get_status: DEMO/LIVE mode + which keys are present
    - revive_get_attention: journeys flagged for human review / high value
    - revive_audit_verify: hash-chain integrity check (chain_ok + last hash)
    - revive_get_guardian_stats: veto counts grouped by reason

Run from main/:  python scripts/run_mcp.py
"""
...
from revive.events import AGG_JOURNEY, E_INTERVENTION_VETOED
...
from revive.store.journey_repo import JourneyRepo, journey_to_dict
...
@mcp.tool()
def revive_list_journeys(limit: int = 50) -> list[dict[str, Any]]:
...
@mcp.tool()
def revive_get_timeline(journey_key: str) -> dict[str, Any]:
...
@mcp.tool()
def revive_get_metrics() -> dict[str, Any]:
...
@mcp.tool()
def revive_list_dead_letters(limit: int = 20) -> list[dict[str, Any]]:
...
@mcp.tool()
def revive_get_status() -> dict[str, Any]:
...
@mcp.tool()
def revive_get_attention(limit: int = 8) -> list[dict[str, Any]]:
...
@mcp.tool()
def revive_audit_verify() -> dict[str, Any]:
...
@mcp.tool()
def revive_get_guardian_stats() -> dict[str, Any]:
